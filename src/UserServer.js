...
//==初始化 TokenDB(mongodb)
UserManager.connectTokenDB('localhost',27017,'usersample')
.then(function(db){
	logger("成功連接至MongoDB!!");
});
...
//服務器配置
//Token有效時長(秒)
UserManager.setTokenExpiration(10);
...
app_server.post('/login',function(req, res){
	...
	logger('[/login]收到資料:'+JSON.stringify(req.body));
